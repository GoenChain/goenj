...
public class Bucket {
    ...
    public synchronized void insert(NodeContract nodeContract) {
        if (nodeContactsSet.contains(nodeContract)) {
            NodeContract tmp = removeFromContactsWithNoRep(nodeContract.getNode());
            tmp.refresh();
    ...
    public NodeContract getNodeContract(Node node) {
        NodeContract cellNodeConstract = this.nodeContactsSet.ceiling(new NodeContract(node));
        if (cellNodeConstract != null && FastByteComparisons.compare(cellNodeConstract.getNode().getNodeId(), node.getNodeId())) {
            return cellNodeConstract;
        } else {
            return null;
        }
    }
    ...
    private synchronized NodeContract removeFromContactsWithNoRep(Node node) {
